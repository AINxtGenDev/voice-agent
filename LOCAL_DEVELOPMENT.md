...
- Agent and customer transcripts are held in memory during the call. After the call, `gpt-5.6-terra` (`store: false`, default reasoning effort medium) summarizes them and a Markdown report is written to `reports/` (owner-only, ignored by Git) with date, time (Europe/Vienna), duration, contact, product, outcome, and meeting details. Duration comes from Twilio's `CallDuration`, or from local timing if that callback does not arrive within 20 seconds. The transcript itself is not stored. If the customer objects to notes or declines the conversation, only metadata is written. A failed summary still writes a metadata report.
...
## QuickSpecs knowledge — 25 September 2026

- Three topics are supported: HPE Private Cloud AI, HPE Alletra Storage MP X10000, and HPE Aruba Networking CX 6300. The knowledge search is scoped to the selected topic, so facts never mix products.
- 41 reviewed facts in `src/hpe-quickspecs-facts.mjs` come from the QuickSpecs PDFs: Private Cloud AI a50009216enw V11 (2026-07-06), Alletra MP X10000 a50009215enw V11 (2026-09-08), CX 6300 a00073540enw V46 (2026-08-03). PDFs were downloaded through the browser (HPE rejects command-line downloads) and extracted with the `hpe-quickspecs` skill's parser. Each fact carries a verbatim quote and page citation; every quote and every number was checked against the cited pages. Prices and performance figures were excluded.
- Known document inconsistencies are stated with page references rather than resolved: Alletra lists 16 JBOFs (pages 2, 5) but "eight storage shelves" (page 11), and 4 vs. 10+2 accelerator nodes.
- Private Cloud AI hardware questions are now answered from QuickSpecs V11, with the notice that the Developer Portal describes a different configuration.
- The `hpe-quickspecs` skill's library search returned only generic navigation links at the time; it was repaired afterwards (outside this repository) and was not used for these facts.
